...
**期間**: 15週間（Phase 0: 2-3日 + Phase 1-3: 15週）+ 追加実験（任意）
...
各Phaseで実装する最適化手法の概要です。詳細は各セクションを参照してください。各Phaseで共通の進め方は`docs/workflow.md`にまとめています。

### Phase 0: ベースライン構築（Week 0: 2-3日）

| 項目 | 内容 |
|------|------|
| データ収集 | LangChainドキュメント200-300ページのスクレイピング |
| 素朴なRAG実装 | FAISS + GPT-4o-mini による基本的なQAシステム |
| テストセット作成 | LangChain特化の質問10問を作成 |
| ベースライン測定 | コスト、レイテンシ、精度の初期値を記録 |

### Phase 1: ベストプラクティス（Week 1-6）

| Week | 手法 | 最適化対象 | 期待効果 |
|------|------|-----------|---------|
| 1 | 計測サーバー構築 | 基盤 | 全ての最適化効果を可視化 |
| 2 | セマンティックキャッシング | コスト・レイテンシ | コスト20-50%削減、レイテンシ95%削減 |
| 3 | ハイブリッド検索（RAG） | 精度 | ハルシネーション30-50%削減 |
| 4 | 構造化出力 + ストリーミング | コスト・UX | 出力トークン10-20%削減、体感レイテンシ50-80%削減 |
| 5 | プロンプトキャッシング | コスト | 静的プロンプト部分90%削減 |
| 6 | バッチプロンプティング | コスト・スループット | API呼び出し回数30-50%削減 |

**Phase 1終了時の累積効果**: コスト50%削減、レイテンシ60%短縮、精度25%向上

### Phase 2: 先進的手法（Week 7-11）

| Week | 手法 | 最適化対象 | 期待効果 |
|------|------|-----------|---------|
| 7-8 | モデルルーティング | コスト | コスト50-75%削減（品質95%維持） |
| 9 | リランキング（Cross-Encoder） | 精度 | ハルシネーション40-60%削減 |
| 10 | プロンプト圧縮（LLMLingua） | コスト | 入力トークン20-50%削減（長文） |
| 11 | 動的Few-shot選択 | 精度 | 複雑タスクで15-30%精度向上 |
| 11+ (Optional) | LLM as Judge（自己評価） | 精度・信頼性 | ハルシネーション/根拠不一致の検出率向上 |

**Phase 2終了時の累積効果**: コスト60%削減、精度40%向上

### Phase 3: 実験的手法（Week 12-15）

| Week | 手法 | 最適化対象 | 期待効果・トレードオフ |
|------|------|-----------|---------------------|
| 12 | ファインチューニング（API経由） | 精度 | ドメイン特化で20-40%改善、データ準備コスト高 |
| 13 | GraphRAG | 精度 | Global Query精度2倍以上、構築コスト高 |
| 14 | エージェンティックワークフロー | 精度 | 複雑推論で大幅改善、コスト・レイテンシ3-10倍 |
| 15 | 並列API呼び出し + アンサンブル | 精度 | 5-20%精度向上、コストN倍 |

**Phase 3終了時の最終成果**: コスト65%削減、レイテンシ70%短縮、精度50%向上（Optional項目は含まない）

### Phase 3+: 追加実験（Optional, Week 16-18）

| Week | 手法 | 最適化対象 | 期待効果・トレードオフ |
|------|------|-----------|---------------------|
| 16 | Adaptive Retrieval（条件付き検索制御） | コスト・レイテンシ | 低難度クエリで検索を省略、コスト削減 |
| 17 | Learned Sparse Retrieval（SPLADE/uniCOIL） | 精度 | BM25より高精度、計算コスト増 |
| 18 | Temporal-aware GraphRAG | 精度 | 更新頻度が高いドキュメントで新鮮性向上 |
...
### 2.5 LLM as Judge（自己評価）

**優先度**: 🟡 高（Optional）
**最適化対象**: 精度・信頼性
**ROI**: ハルシネーションや根拠不一致の検出率向上

#### 実験内容

生成回答に対して、別のLLMに「根拠との整合性」や「答えの妥当性」を採点させます。評価結果に応じて再検索・再生成・上位モデルへのフォールバックを行い、品質を担保します。

#### LangChain Explorerでの適用

- **RAG回答の根拠検証**: 取得ドキュメントと回答の整合性を判定
- **低スコア時の再検索**: 追加のTop-k取得や再ランキングに回す

#### 実装手順

1. Judge用の評価プロンプト（採点ルーブリック）を定義
2. JSON形式で `score`, `is_grounded`, `reason` を出力
3. スコア閾値でフォールバック/再検索を制御

#### Pythonツールセット

```python
openai              # Judgeモデル（gpt-4o-mini等）
anthropic           # Claude系のJudge
pydantic            # 評価結果の型定義
instructor          # JSON出力の強制
```

#### 検証方法

* 人間評価とJudge評価の一致率を測定
* 不合格回答の再生成で精度が上がるかを比較
...
### 3.5 Adaptive Retrieval（条件付き検索制御）
...
クエリの難易度や不確実性に応じて、検索をスキップしたりTop-kを動的に変更します。
「明らかな定義質問」は検索せず、「複雑な比較質問」はTop-kを増やす、といった適応制御を行います。

#### 実装手順

1. クエリ分類（簡単/普通/難しい）を実装
2. 難易度に応じて `retriever_k` や再検索を切り替え
3. 低難度クエリは検索を省略し、コスト削減
...
### 3.6 Learned Sparse Retrieval（SPLADE / uniCOIL）

**優先度**: 🟢 低（Optional）
**最適化対象**: 精度
**ROI**: BM25より高精度だが計算コスト増

#### 実験内容

学習済みスパースリトリーバ（SPLADE等）を使い、
キーワード検索の利点を保ちながら精度を向上させます。

#### 実装手順

1. SPLADE/uniCOILのモデルを選定
2. スパース表現を生成し、インデックス化
3. 既存のベクトル検索と統合（ハイブリッド化）

#### 検証方法

* BM25とのPrecision/Recall比較
* CPU実行時のレイテンシとコストを計測

---

### 3.7 Temporal-aware GraphRAG（時間軸対応）

**優先度**: 🟢 低（Optional）
**最適化対象**: 精度
**ROI**: 更新頻度が高いドキュメントで新鮮性向上

#### 実験内容

GraphRAGに「更新日」「版数」などの時間メタデータを付与し、
新しい情報を優先するグラフ検索を実装します。

#### 実装手順

1. ノード/エッジにタイムスタンプを付与
2. 新しい情報を優先するランキング関数を導入
3. 更新頻度の高いページで効果測定

#### 検証方法

* 更新直後のドキュメントで正答率が向上するかを確認
* 時間重みの有無で精度比較
...
## 総合ロードマップ：LangChain Explorer 開発スケジュール（15週間 + 追加実験）
...
| **Week 11+ (Optional)** | Phase 2-5 | **LLM as Judge** | ✅ ルーブリック付き評価プロンプトを設計<br>✅ 根拠不一致を検出し再検索にフォールバック<br>✅ 人間評価との一致率を測定 | 信頼性の向上 |
...
| **Week 16 (Optional)** | Phase 3+ | **Adaptive Retrieval** | ✅ 難易度分類で検索有無を切替<br>✅ Top-kの動的制御を実装<br>✅ コスト/精度トレードオフを測定 | 低難度クエリの<br>コスト削減 |
| **Week 17 (Optional)** | Phase 3+ | **Learned Sparse Retrieval** | ✅ SPLADE/uniCOILでスパース表現生成<br>✅ BM25との比較評価<br>✅ ハイブリッド検索に統合 | キーワード検索の<br>精度向上 |
| **Week 18 (Optional)** | Phase 3+ | **Temporal-aware GraphRAG** | ✅ 時間メタデータを付与<br>✅ 新鮮性重視のランキングを実装<br>✅ 更新頻度の高いページで検証 | 新鮮性の向上 |

Optional（Week 16-18）は、時間が確保できる場合のみ実施します。Week 15終了時点の成果を最優先とします。
...
LLMシステム最適化の15種類の手法（+ 追加実験）を、単なる理論として学ぶのではなく、**LangChain公式ドキュメントQAボット**という実際のプロダクト構築を通じて習得します。
...
15週間後（必要に応じて追加実験後）、あなたは以下を手にしています：

- ✅ 実用的なLangChain QAボット
- ✅ 15種類の最適化手法の実装経験（+ 追加実験）
- ✅ 週次実験ログ15本（ポートフォリオ）
- ✅ 実務で即座に活用できる知識とコード
